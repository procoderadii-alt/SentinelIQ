...
from fastapi import Depends, FastAPI, HTTPException, Query, Response, WebSocket, WebSocketDisconnect, status, UploadFile, File
...
def startup() -> None:
    ...
    with Session(bind=engine) as db:
        # Default seed count
        seed_database(db, crimes=int(os.getenv("SEED_CRIMES", "2000")))
...
def require(permission_name: str):
...
def login(
    ...
    permissions = [p.name for p in user.role.permissions]
    token = create_access_token(user.email, user.role.name)
...
def dashboard(_: User = Depends(require("View Data")), db: Session = Depends(get_db)):
...
def list_crimes(
    ...
    _: User = Depends(require("View Data")),
    db: Session = Depends(get_db)
):
...
async def create_crime(
    payload: CrimeIn,
    user: User = Depends(require("Edit Data")),
    db: Session = Depends(get_db)
):
    ...
    log_audit(db, user.email, "CREATE", "CrimeRecord", row.crime_id)
    invalidate_cache()
    db.commit()
    db.refresh(row)
...
def update_crime(
    ...
    log_audit(db, user.email, "UPDATE", "CrimeRecord", crime_id)
    invalidate_cache()
    db.commit()
    db.refresh(row)
    return row_to_crime(row)
...
def delete_crime(
    crime_id: str,
    user: User = Depends(require("Delete Data")),
    db: Session = Depends(get_db)
):
    row = db.scalar(select(CrimeRecord).where(CrimeRecord.crime_id == crime_id))
    if not row:
    ...
    db.delete(row)
    log_audit(db, user.email, "DELETE", "CrimeRecord", crime_id)
    invalidate_cache()
    db.commit()
    return Response(status_code=204)

@app.get("/api/gis/geojson")
def geojson(_: User = Depends(require("View Data")), db: Session = Depends(get_db)):
...
def get_geojson_districts(_: User = Depends(require("View Data")), db: Session = Depends(get_db)):
...
def hotspots(_: User = Depends(require("Access AI Services")), db: Session = Depends(get_db)):
    return ai_services.detect_hotspots(db, limit=300)

@app.get("/api/analytics/anomalies")
def anomalies(_: User = Depends(require("Access AI Services")), db: Session = Depends(get_db)):
    return ai_services.detect_anomalies(db, limit=300)

@app.post("/api/search")
def natural_language_search(payload: dict, _: User = Depends(require("View Data")), db: Session = Depends(get_db)):
...
def copilot_chat(payload: dict, _: User = Depends(require("Access AI Services")), db: Session = Depends(get_db)):
    query = payload.get("query", "")
...
def create_report_record(payload: dict, user: User = Depends(require("Generate Reports")), db: Session = Depends(get_db)):
...
def export_csv(_: User = Depends(require("Generate Reports")), db: Session = Depends(get_db)):
    buffer = io.StringIO()
...
def export_excel(_: User = Depends(require("Generate Reports")), db: Session = Depends(get_db)):
    if not HAS_PANDAS:
...
def export_pdf(_: User = Depends(require("Generate Reports")), db: Session = Depends(get_db)):
...
async def upload_dataset(
    ...
    if user.role.name != "Admin":
        raise HTTPException(status_code=403, detail="Only Admin can upload datasets.")
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Failed to decode CSV. Must be UTF-8.")
    ...
    for row in reader:
        records_uploaded += 1
        cid = row.get("crime_id", "").strip()
        if not cid or cid in existing_crimes:
            duplicates_removed += 1
            continue

        d_name = row.get("district", "Unknown District").strip()
        c_type = row.get("crime_type", "Other").strip()
        
        try:
            lat = float(row.get("latitude", 0.0))
            lng = float(row.get("longitude", 0.0))
            dt = datetime.fromisoformat(row.get("date").replace('Z', '+00:00'))
        except Exception:
            duplicates_removed += 1
            continue

        if d_name not in districts:
            districts[d_name] = District(id=uuid.uuid4(), name=d_name, code=f"DST-{len(districts)}", population=250000, latitude=lat, longitude=lng)
    ...
    return DatasetUploadSummary(
        ...
        duplicates_removed=records_uploaded - valid_records,
        districts_updated=len(districts),
        hotspots_recalculated=math.ceil(valid_records / 100),
        processing_time=f"{elapsed:.2f} seconds"
    )

@app.get("/api/datasets/history", response_model=List[DatasetOut])
def dataset_history(_: User = Depends(current_user), db: Session = Depends(get_db)):
