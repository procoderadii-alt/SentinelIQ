...
const alertColor: Record<string, string> = {
  ...
  High: "border-amber-500/50 bg-amber-500/20 text-amber-300",
  Medium: "border-emerald-500/50 bg-emerald-500/20 text-emerald-300",
  Low: "border-blue-500/50 bg-blue-500/20 text-blue-300",
};
import { DashboardData, fetchDashboard, naturalLanguageSearch, askCopilot, downloadReport, uploadDataset, fetchDatasetHistory, DatasetHistory, DatasetUploadSummary, fetchIncidents } from "./api";
...
function useDashboard() {
  ...
  if (!ctx) throw new Error("Dashboard data not loaded yet");
  return ctx;
}
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_340px]">
        <svg viewBox="-180 -150 460 360" className="h-[520px] w-full rounded border border-white/10 bg-[#061114]">
          ...
          {networkNodes.map((node) => (
            <g key={node.id}>
              <title>{node.label} - Weight: {node.weight}</title>
...
function CCTV() {
  ...
  return (
    <Card>
      <ModuleHeader icon={Camera} title="CCTV & Surveillance" subtitle="Live simulated camera feeds with real-time AI tracking overlays" />
      <div className="mt-5 grid gap-4 grid-cols-1 md:grid-cols-2">
        {[1, 2, 3, 4].map((camIndex) => (
          <div key={camIndex} className="relative rounded overflow-hidden border border-white/10 bg-[#061114] aspect-video flex flex-col items-center justify-center">
            {/* Simulated Video Feed Background */}
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-slate-800 to-[#061114] opacity-50 animate-pulse"></div>
            
            {/* Camera Overlay HUD */}
            <div className="absolute top-3 left-3 flex items-center gap-2">
              <div className="size-2 rounded-full bg-red-500 animate-ping"></div>
              <span className="text-xs font-mono text-white/80">CAM-{String(camIndex).padStart(2, '0')} REC</span>
            </div>
            <div className="absolute top-3 right-3 text-xs font-mono text-white/50">
              {new Date().toISOString().split('T')[1].substring(0, 8)}
            </div>
            
            {/* Crosshair Graphic */}
            <div className="relative size-32 border border-cyanline/30 rounded-sm">
              <div className="absolute top-1/2 left-0 right-0 h-[1px] bg-cyanline/20"></div>
              <div className="absolute left-1/2 top-0 bottom-0 w-[1px] bg-cyanline/20"></div>
            </div>

            {/* AI Event Overlay */}
            {cctvEvents[camIndex - 1] && (
              <div className="absolute bottom-0 inset-x-0 bg-black/60 backdrop-blur-sm p-3 border-t border-cyanline/30">
                <p className="text-xs text-cyanline font-semibold">AI Detection Event</p>
                <p className="text-sm text-slate-200 truncate">{cctvEvents[camIndex - 1]}</p>
                <div className="mt-1 w-full bg-white/10 h-1 rounded-full overflow-hidden">
                   <div className="bg-cyanline h-full" style={{ width: `${80 + Math.random() * 15}%` }}></div>
                </div>
              </div>
            )}
          </div>
...
function SearchAndReports({ reports = false }: { reports?: boolean }) {
  ...
  function downloadCSV() {
    const dataToExport = results.length > 0 ? results : useDashboard().incidents;
    if (!dataToExport || dataToExport.length === 0) {
      setStatus("No data available to export.");
      return;
    }

    const headers = ["ID", "FIR", "Category", "District", "Status", "Latitude", "Longitude", "Occurred At"];
    ...
    link.setAttribute("download", `sentineliq_export_${new Date().getTime()}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    
    setStatus("CSV Export generated successfully.");
  }
    ...
    <Card>
      ...
      {reports && (
        <div className="mt-4 flex flex-wrap gap-3">
          <button className="btn" onClick={downloadCSV}><Download size={16} /> Export as CSV</button>
        </div>
      )}
          ...
          {summary && (
            <Card className="p-6 border-green-500/30 bg-green-500/5">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className={`text-xl font-bold ${summary.data_quality_score >= 90 ? 'text-green-400' : summary.data_quality_score >= 70 ? 'text-yellow-400' : 'text-red-400'}`}>
                  {summary.data_quality_score.toFixed(1)}% Q-Score
                </div>
              </div>
              <ul className="space-y-3 text-sm text-slate-300">
                ...
                <li className="flex justify-between"><span>Valid Records:</span> <span className="font-mono text-green-400">{fmt(summary.valid_records)}</span></li>
                <li className="flex justify-between"><span>Duplicates Removed:</span> <span className="font-mono text-slate-400">{fmt(summary.duplicates_removed)}</span></li>
                <li className="flex justify-between"><span>Missing Values Dropped:</span> <span className="font-mono text-orange-400">{fmt(summary.missing_values)}</span></li>
                <li className="flex justify-between"><span>Anomalies Removed:</span> <span className="font-mono text-red-400">{fmt(summary.anomalies_detected)}</span></li>
              </ul>
              <div className="mt-4 pt-4 border-t border-slate-700 text-xs text-slate-500 flex justify-between">
                <span>Districts Processed: {summary.districts_updated}</span>
                <span>Processing Time: {summary.processing_time}</span>
              </div>
            </Card>
          )}
        </div>
...
export function App() {
  ...
  const [wsToast, setWsToast] = useState<{message: string, timestamp: number} | null>(null);
  ...
  useEffect(() => {
    ...
    fetchDashboard()
      .then((data) => {
        if (!cancelled) {
          setDashboard(data);
          setConnection("Live API");
        }
      })
      .catch(() => {
        if (!cancelled) setConnection("Offline / API Error");
      });
...
const ws = new WebSocket(
  `${protocol}://${window.location.hostname}/api/ws/alerts`
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (payload.type === "dataset_imported" || payload.type === "alert") {
           setWsToast({ message: payload.message || "New Alert Triggered", timestamp: Date.now() });
  ...
  if (!dashboard) {
    return <div className="flex h-screen items-center justify-center bg-slate-900 text-cyanline">Loading SentinelIQ Database...</div>;
  }
      ...
      <main className="min-h-screen bg-slate-100 text-slate-950 dark:bg-ink dark:text-slate-100">
        ...
        {wsToast && (
          <motion.div 
            ...
            animate={{ opacity: 1, y: 0 }}
            className="fixed top-4 right-4 z-50 bg-[#0c1a1f] border border-cyanline rounded shadow-lg p-4 flex items-center gap-3 text-sm text-cyanline"
          >
            <ShieldAlert size={18} className="text-cyanline" />
            <span>{wsToast.message}</span>
          </motion.div>
        )}
      </main>
    </div>
